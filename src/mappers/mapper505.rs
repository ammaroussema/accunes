...
impl Mapper for Mapper505 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let offset = if (self.reg & 0x01) != 0 {
                let page = (address as usize - 0x8000) / 0x2000;
                let prg_and = 0x1F;
                let prg_or = (self.reg & 0x60) as usize;
    ...
    fn fetch_ppu(
        ...
        let chr_and = if (self.reg & 0x100) != 0 { 0x7F } else { 0xFF };
        let chr_or = (((self.reg >> 1) as usize & 0x100) | (self.reg as usize & 0x080)) & !chr_and;
