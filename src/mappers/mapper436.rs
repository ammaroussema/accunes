...
impl Mapper436 {
    ...
    fn banking(&self) -> (u16, u16, usize, usize) {
        if self.submapper == 1 {
            let cond = (self.core.reg4100[0x1C] & 0x01) != 0 && (self.core.reg4100[0x1C] & 0x04) == 0;
            let prg_or = if cond { 0x0800 } else { 0 };
            let chr_or = if cond { 0x4000 } else { 0 };
            (0xF7FF, prg_or, 0xBFFF, chr_or)
        } else {
            let bit_f = (self.core.reg4100[0x0F] & 0x20) != 0;
            let prg_or = (if bit_f { 0x0400 } else { 0 })
                | (if (self.core.reg4100[0x00] & 0x40) != 0 { 0x0800 } else { 0 });
            let chr_or = (if bit_f { 0x2000 } else { 0 })
                | (if (self.core.reg4100[0x00] & 0x04) != 0 { 0x4000 } else { 0 });
            (0xF3FF, prg_or, 0x9FFF, chr_or)
        }
    }
}

impl Mapper for Mapper436 {
    fn reset(&mut self) {
        self.core.reset();
    }
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if address >= 0x8000 {
            let (prg_and, prg_or, _, _) = self.banking();
            let slot = ((address - 0x8000) >> 13) as usize;
            let bank = ((self.core.get_prg_bank(slot) as u16 & prg_and) | prg_or) as usize;
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            let (_, _, chr_and, chr_or) = self.banking();
            let mut banking = self.core.banking;
            banking.chr_and = chr_and;
            banking.chr_or = chr_or;
