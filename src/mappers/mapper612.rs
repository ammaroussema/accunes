...
impl Mapper612 {
    pub fn new() -> Self {
        Self { reg: 0, latch: 0, soft_reset_count: 0 }
    }

    fn prg_16k_banks(&self) -> (usize, usize) {
        let base = (self.reg as usize) << 3;
        let bank0 = base | ((self.latch as usize) & 0x07);
        let bank1 = base | 0x07;
        (bank0, bank1)
    }
}

impl Mapper for Mapper612 {
    fn reset(&mut self) {
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> {
        vec![self.latch, self.reg, self.soft_reset_count]
    }
