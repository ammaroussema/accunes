...
fn transform_mmc3_addr(submapper: u8, address: u16) -> u16 {
    let low = (address & 0xFF) as u8;
    let transformed = match submapper {
        1 | 3 => low >> 1,
        2 => low >> 2,
        _ => (if low & 0x0E != 0 { 1u8 } else { 0u8 }) ^ (low & 1),
    };
    (address & 0xE000) | (transformed as u16 & 1)
}
...
impl Mapper for Mapper196 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if self.submapper == 3 {
            let bank = ((self.reg >> 4) as usize) % (prg_len / 0x8000).max(1);
            let offset = bank * 0x8000 + (address as usize & 0x7FFF);
            return FetchResult { data: cart.prg_rom[offset % prg_len], driven: true };
        }
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        ...
        if address >= 0x8000 {
            let fake = transform_mmc3_addr(self.submapper, address);
            self.mmc3.store_prg(cart, fake, data);
        }
    }
