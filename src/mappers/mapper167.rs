...
impl Mapper for Mapper167 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            let prg = ((self.reg[0] ^ self.reg[1]) << 1 & 0x20) | (self.reg[2] ^ self.reg[3]) & 0x1F;
            let bank16 = match (self.reg[1] >> 2) & 3 {
                0 => prg,
                1 => 0x1F,
                _ => if address < 0xC000 { prg | 1 } else { prg & !1 },
            };
            let offset = if address < 0xC000 {
                let bank = if (self.reg[1] >> 2) & 3 == 1 { prg } else { bank16 };
                bank as usize * 0x4000 + (address as usize & 0x3FFF)
            } else {
                let bank = if (self.reg[1] >> 2) & 3 == 0 { 0x20 } else { bank16 };
                bank as usize * 0x4000 + (address as usize & 0x3FFF)
            };
