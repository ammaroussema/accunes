...
impl Mapper366 {
    pub fn new(header: &[u8], _rom: &[u8], _rom_name: &str) -> Self {
        ...
        Self { mmc3: MapperMMC3::new(config), outer_bank: 0 }
    }
}

impl Mapper for Mapper366 {
    fn reset(&mut self) {
        self.outer_bank = 0;
        self.mmc3.reset();
    }
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let bank = (mmc3_bank & 0x0F) | (self.outer_bank as usize);
    ...
    fn store_prg(&mut self, _cart: &mut Cartridge, address: u16, _val: u8) {
        if address >= 0x6000 && address < 0x8000 && (address & 0x0080) == 0 {
            self.outer_bank = (address & 0x30) as u8;
            self.mmc3.reset();
        }
    }
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            if using_chr_ram && !chr_ram.is_empty() {
                ...
                let bank = (chr_bank & 0x7F) | ((self.outer_bank as usize) << 3);
    ...
    fn load_mapper_registers(&mut self, cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        ...
        if p < state.len() { self.outer_bank = state[p]; p + 1 } else { p }
    }
}
