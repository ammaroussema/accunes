...
impl Mapper for Mapper582 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            let bank_low = if address < 0xC000 {
                (self.reg[0] & 7) | (self.reg[1] << 3)
            } else {
                (self.reg[2] & 7) | (self.reg[3] << 3)
            };
            let offset = (bank_low as usize) * 0x4000 + (address as usize & 0x3FFF);
