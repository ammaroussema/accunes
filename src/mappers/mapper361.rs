...
impl Mapper361 {
    pub fn new(header: &[u8], rom: &[u8], rom_name: &str) -> Self {
        ...
        Self { mmc3: MapperMMC3::new(config), reg: 0, lock: 0 }
    }
}

impl Mapper for Mapper361 {
    fn reset(&mut self) {
        self.reg = 0;
        self.lock = 0;
        self.mmc3.reset();
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if address >= 0x8000 {
            ...
            let prg_or = (self.reg as usize) & !0x0F;
            mmc3_bank = (mmc3_bank & 0x0F) | prg_or;
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            if using_chr_ram && !chr_ram.is_empty() {
                ...
                let bank = (chr_bank & 0x7F) | ((self.reg as usize) << 3 & !0x7F);
    ...
    fn load_mapper_registers(&mut self, cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        ...
        if p < state.len() {
            self.reg = state[p];
            let p = p + 1;
            if p < state.len() { self.lock = state[p]; p + 1 } else { p }
        } else { p }
    }
}
