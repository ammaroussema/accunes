...
impl Mapper579 {
    ...
    fn sync(&mut self) {
        if self.addr & 0x200 != 0 {
            self.addr_locked = 0xFFE3;
        } else {
...
impl Mapper for Mapper579 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        let prg = (self.addr >> 2) & 0x1F;
        if self.addr & 0x200 != 0 {
            let page = if address >= 0xC000 { prg | 7 } else { prg };
            let offset = (page as usize) * 0x4000 + (address as usize & 0x3FFF);
            FetchResult {
                data: cart.prg_rom[offset % cart.prg_rom.len()],
                driven: true,
            }
        } else if self.addr & 0x001 != 0 {
