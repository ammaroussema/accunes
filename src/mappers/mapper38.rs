...
impl Mapper for Mapper38 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let offset = (bank * 0x8000 + (address as usize & 0x7FFF)) % len;
            FetchResult {
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            let bank = ((self.bank_select >> 2) & 0x03) as usize;
            if using_chr_ram {
                let len = chr_ram.len();
                if len > 0 {
                    new_addr_bus |= chr_ram[(bank * 0x2000 + (address as usize & 0x1FFF)) % len] as u16;
                }
            } else {
                let len = chr_rom.len();
                if len > 0 {
