...
impl Mapper for Mapper510 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let prg_or = (if (self.reg & 0x08) != 0 { 0x020 } else { 0 })
                | (if (self.reg & 0x40) != 0 { 0x010 } else { 0 });
            let bank = (self.mmc3_raw_bank(cart, page) & prg_and) | (prg_or & !prg_and);
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if (0x6000..0x8000).contains(&address) {
            if (self.reg & 0x80) == 0 && (self.mmc3.prg_ram_protect & 0x40) == 0 {
                self.reg = data | 0x80;
            }
        }
    ...
    fn fetch_ppu(
        ...
        let chr_and = if (self.reg & 0x01) != 0 { 0xFF } else { 0x7F };
        let chr_or = (if (self.reg & 0x04) != 0 { 0x100 } else { 0 })
            | (if (self.reg & 0x02) != 0 { 0x080 } else { 0 });
