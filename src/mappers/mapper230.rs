...
impl Mapper for Mapper230 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            let offset = if self.reset_flag != 0 {
                let bank = (self.latch & 7) as usize;
                if address < 0xC000 {
                    bank * 0x4000 + (address as usize & 0x3FFF)
                } else {
                    7usize * 0x4000 + (address as usize & 0x3FFF)
                }
            } else {
                let bank = ((self.latch & 0x1F) + 8) as usize;
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            ...
            let mirrored = if self.reset_flag != 0 {
                address & 0x37FF
            } else if self.latch & 0x40 != 0 {
                address & 0x37FF
            } else {
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> {
        vec![self.latch, self.reset_flag]
    }

    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        if start < state.len() { self.latch = state[start]; }
        if start + 1 < state.len() { self.reset_flag = state[start + 1]; }
        start + 2
    }
}
