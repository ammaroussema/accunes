...
impl Mapper443 {
    ...
    fn prg_bank8(&self, cart: &Cartridge, cpu_bank: u8) -> u8 {
        let prg_or = (((self.reg & 0x02) << 4) | (self.reg & 0x10)) & !0x0F;
        (self.get_prg_page(cart, cpu_bank) & 0x0F) | prg_or
    }

    fn chr_bank(&self, address: u16) -> u16 {
        let raw = self.mmc3.chr_bank(address) as u16;
        (raw & 0x00FF) | ((self.reg as u16) << 8)
    }
}

impl Mapper for Mapper443 {
    fn reset(&mut self) {
        self.mmc3.reset();
        self.reg = 0;
    }
