...
use crate::mappers::one_bus_gpio::{AttachedDevice, DeviceBinding, SerialBinding, SerialRomDevice};

pub struct Mapper426 {
    core: OneBus,
}

impl Default for Mapper426 {
    fn default() -> Self {
        Self::new(&[])
    }
}
...
impl Mapper426 {
    pub fn new(rom_data: &[u8]) -> Self {
        ...
        let serial_rom = if rom_data.len() >= 0x100 {
            rom_data[rom_data.len() - 0x100..].to_vec()
        } else {
            vec![0; 256]
        };
        core.gpio[2].devices.push(AttachedDevice {
            device: Box::new(SerialRomDevice::new(serial_rom)),
