...
impl Mapper for Mapper583 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        let prg = (self.addr & !0xF) | ((self.addr << 3) & 0x08);
        if self.addr & 0x20 != 0 {
            let page = ((prg >> 1) & !7 | (self.data & 7) as u16) as usize;
            let offset = page * 0x8000 + (address as usize & 0x7FFF);
            FetchResult {
                data: cart.prg_rom[offset % cart.prg_rom.len()],
                driven: true,
            }
        } else if self.addr & 0x10 != 0 && self.addr & 0x08 != 0 {
            let page = if address >= 0xC000 {
                (prg | 3) as usize
            } else {
                (prg | (self.data & 3) as u16) as usize
            };
            ...
            let page = if address >= 0xC000 {
                ...
                ((prg & !7) | (self.data & 7) as u16) as usize
            };
            let offset = page * 0x4000 + (address as usize & 0x3FFF);
            FetchResult {
                data: cart.prg_rom[offset % cart.prg_rom.len()],
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if address >= 0x6000 && address < 0x8000 {
            ...
            if self.addr & 0x80 == 0 && address & 0x80 != 0 {
                self.addr_locked = 0;
                self.data_locked = 0;
            }
