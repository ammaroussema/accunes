...
pub struct Mapper521 {
    prg: u8,
    prg_size: u8,
}

impl Mapper521 {
    pub fn new(prg_size: u8) -> Self {
        Self { prg: 0, prg_size }
    }
}

impl Mapper for Mapper521 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            if address < 0xC000 {
                ...
                let fixed = (self.prg_size as usize).saturating_sub(1);
                let offset = fixed * 0x4000 + (address as usize & 0x3FFF);
                FetchResult { data: cart.prg_rom[offset % cart.prg_rom.len()], driven: true }
            }
        } else {
    ...
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        if start < state.len() { self.prg = state[start]; start + 1 } else { start }
    }
