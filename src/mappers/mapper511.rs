...
impl Mapper for Mapper511 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let effective_addr = if (self.reg & 0x08) != 0 {
                (address & !3) | (self.mmc3.get_dip_switches() as u16 & 3)
            } else {
                address
            };
            let page = (effective_addr as usize - 0x8000) / 0x2000;
            let raw_bank = if (self.reg & 0x04) != 0 {
                let mask = if (self.reg & 0x02) != 0 { 1 } else { 3 };
                (self.mmc3_raw_bank(cart, page & 1) & !mask) | (page & mask)
            } else {
                self.mmc3_raw_bank(cart, page)
            };
            let prg_and = 0x0F;
            let prg_or = ((self.reg as usize) << 4) & !prg_and;
            let bank = (raw_bank & prg_and) | prg_or;
            let offset = bank * 0x2000 + (effective_addr as usize & 0x1FFF);
            return FetchResult {
