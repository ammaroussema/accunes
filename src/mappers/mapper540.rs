...
impl Mapper540 {
    ...
    fn chr_bank_2k(&self, address: u16) -> usize {
        match address {
            0x0000..=0x07FF => self.chr[0] as usize,
            0x0800..=0x0FFF => self.chr[1] as usize,
            0x1000..=0x17FF => self.chr[6] as usize,
...
impl Mapper for Mapper540 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        let bank_raw = if address >= 0xE000 {
            0xFF
        } else {
            self.prg[page]
        };
    ...
    fn store_prg(&mut self, _cart: &mut Cartridge, address: u16, val: u8) {
        match address & 0xF000 {
            0x8000 => {
                self.prg[(address & 3) as usize] = val;
            }
            0x9000 => match address & 3 {
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            let bank = self.chr_bank_2k(address);
    ...
    fn ppu_clock(
        ...
        if rising {
            if self.pa12_filter == 0 && self.irq_pa12 {
                let low = self.counter as u8;
                let high = (self.counter >> 8) as u8;
                if low == 0 || self.reload {
                    self.counter = (self.counter & 0xFF00) | (high as u16);
                } else {
                    self.counter = (self.counter & 0xFF00) | ((low.wrapping_sub(1)) as u16);
                }
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> {
        ...
        state.push(
            self.irq_enabled as u8
                | (self.irq_pa12 as u8) << 1
                | (self.irq_auto_enable as u8) << 2,
        );
    ...
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        ...
        if p < state.len() {
            let f = state[p];
            p += 1;
            self.irq_enabled = (f & 1) != 0;
            self.irq_pa12 = (f & 2) != 0;
            self.irq_auto_enable = (f & 4) != 0;
        }
        if p < state.len() {
