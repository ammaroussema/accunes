...
impl Mapper573 {
    ...
    fn compute_prg_bank(&self, address: u16) -> u16 {
        if self.addr & 0x02 != 0 {
            let base = if address >= 0xC000 {
                0x0Fu16
            } else {
                self.data as u16 & 0x0F
            };
            base | (((self.addr & 0x02) as u16) << 3)
        } else {
            let base = if address >= 0xC000 {
                0x07u16
            } else {
                self.data as u16 & 0x07
            };
            base | (((self.addr & 0x03) as u16) << 3)
        }
    }

    fn read_prg_rom(&self, cart: &Cartridge, address: u16) -> u8 {
        if cart.prg_rom.is_empty() {
            return 0;
        }
        if self.addr & 0x10 != 0 && cart.prg_rom.len() > 128 * 1024 {
            return 0;
        }
        if address < 0x8000 {
            return 0;
        }
        let bank = self.compute_prg_bank(address);
        let offset = (bank as usize) * 0x4000 + (address as usize & 0x3FFF);
        cart.prg_rom[offset % cart.prg_rom.len()]
    }
}

impl Mapper for Mapper573 {
    fn reset(&mut self) {
        self.addr = 0;
        self.data = 0;
        self.addr_locked = 0;
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if address >= 0x8000 {
            let rom_data = self.read_prg_rom(cart, address);
            let new_data = data & rom_data;
            let new_addr = (self.addr & self.addr_locked) | (address & !self.addr_locked);
            let new_data = (self.data & self.data_locked) | (new_data & !self.data_locked);
            self.addr = new_addr;
            self.data = new_data;
            self.sync();
        }
