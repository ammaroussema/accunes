...
impl Mapper for Mapper548 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        match address {
            0x6000..=0x7FFF => {
                ...
                FetchResult { data: cart.prg_ram.get(idx).copied().unwrap_or(0), driven: true }
            }
            0x8000..=0xBFFF => FetchResult { data: self.prg_read_bank16(self.reg, address, &cart.prg_rom), driven: true },
            0xC000..=0xFFFF => FetchResult { data: self.prg_read_bank16(3, address, &cart.prg_rom), driven: true },
    ...
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], mut start: usize) -> usize {
        ...
        if start + 2 <= state.len() { self.counter = u16::from_le_bytes([state[start], state[start + 1]]); start += 2; }
