...
impl Mapper578 {
    ...
    fn prg_and(&self) -> u16 {
        if self.reg & 0x06 == 0x06 { 0x1F } else { 0x0F }
    }

    fn chr_and(&self) -> u16 {
        if self.reg & 0x06 == 0x06 { 0xFF } else { 0x7F }
    }

    fn prg_page(&self, cart: &Cartridge, address: u16) -> u16 {
        let bank8 = ((address as usize) - 0x8000) / 0x2000;
        let raw = self.std_prg_bank(cart, bank8);
        let prg_or = ((self.reg as u16) << 4) & !self.prg_and();
        (raw & self.prg_and()) | prg_or
    }
    ...
    fn chr_page(&self, address: u16) -> u16 {
        ...
        let chr_or = ((self.reg as u16) << 7) & !self.chr_and();
        (raw & self.chr_and()) | chr_or
    }
}

impl Mapper for Mapper578 {
    fn reset(&mut self) {
        self.reg = 0;
        self.mmc3.reset();
    }
