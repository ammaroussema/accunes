...
impl Mapper567 {
    ...
    fn prg_raw_bank(&self, cart: &Cartridge, bank8: usize) -> u16 {
        if self.reg == 0x08 {
            (self.std_prg_bank(cart, bank8 & 1) & !3) | (bank8 as u16 & 3)
        } else {
    ...
    fn chr_page(&self, address: u16) -> u16 {
        if self.reg == 0x0F {
            ...
            (raw & 0x7F) | (((self.reg as u16) << 7) & 0x380)
        }
    }
}

impl Mapper for Mapper567 {
    fn reset(&mut self) {
        self.reg = 0;
        self.mmc3.reset();
    }
