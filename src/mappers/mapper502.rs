...
impl Mapper502 {
    ...
    fn mask(&self) -> usize {
        let mode = ((self.reg[1] >> 4) & 3) as u32;
        ((8u32 << mode) - 1) as usize
    }

    fn mode_32k(&self) -> bool {
        (self.reg[1] & 6) != 0
    }

    fn outer_32k(&self) -> usize {
        (self.reg[0] as usize) << 2
    }

    fn outer_16k(&self) -> usize {
        (self.reg[0] as usize) << 3
    }

    fn single_screen_mode(&self) -> bool {
        (self.reg[1] & 2) != 0
    }
    ...
    fn mirror_address(&self, address: u16) -> usize {
        if self.single_screen_mode() {
            let off = (address & 0x03FF) as usize;
            if self.single_screen_high() { 0x400 | off } else { off }
        } else {
            (mirror_h_or_v(!self.mirror_vertical(), address) & 0x7FF) as usize
        }
    }
}

impl Mapper for Mapper502 {
    fn reset(&mut self) {
        self.latch_data = 0;
    }
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if address >= 0x8000 {
            ...
            let mask = self.mask();
            let offset = if self.mode_32k() {
                let bank = self.outer_32k() + (self.latch_data as usize & (mask >> 1));
                bank * 0x8000 + (address as usize & 0x7FFF)
            } else {
                let bank = if address < 0xC000 {
                    self.outer_16k() + (self.latch_data as usize & mask)
                } else {
                    self.outer_16k() + mask
                };
    ...
    fn mirror_nametable(&self, _cart: &Cartridge, address: u16) -> u16 {
        self.mirror_address(address) as u16
    }
