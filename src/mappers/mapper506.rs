...
impl Mapper for Mapper506 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let prg_and = if (self.reg & 3) != 3 { 0x0F } else { 0x1F };
            let prg_or = (((self.reg as usize) << 5) & 0x20) | (((self.reg as usize) << 3) & 0x10);
    ...
    fn fetch_ppu(
        ...
        let chr_and = if (self.reg & 3) != 0 { 0x7F } else { 0xFF };
        let chr_or = ((((self.reg as usize) << 7) & 0x100) | ((((!self.reg) as usize) << 7) & 0x80)) & !chr_and;
