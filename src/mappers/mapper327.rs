...
impl Mapper for Mapper327 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if address >= 0x8000 {
            let outer = ((self.reg as usize) & 7) << 4;
            let mask = if (self.reg & 0x08) != 0 { 0x1F } else { 0x0F };
            ...
            let bank_mod = (mmc3_bank & mask) | outer;
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            ...
            let use_chr_ram = (self.reg & 0x10) != 0 || using_chr_ram;

            let byte = if use_chr_ram {
                let offset = (chr_bank as usize) * 0x400 + (address as usize & 0x3FF);
                if !chr_ram.is_empty() {
                    chr_ram[offset % chr_ram.len()]
                } else { 0 }
            } else {
                let outer = ((self.reg as usize) & 7) << 7;
                let mask = if (self.reg & 0x20) != 0 { 0xFF } else { 0x7F };
                let bank = (chr_bank as usize & mask) | outer;
    ...
    fn store_ppu(&mut self, cart: &mut Cartridge, address: u16, data: u8, vram: &mut [u8]) {
        if address < 0x2000 && (cart.using_chr_ram || (self.reg & 0x10) != 0) {
            let chr_bank = mmc3_chr_bank(
                self.mmc3.r8000, self.mmc3.chr_2k0, self.mmc3.chr_2k8,
                self.mmc3.chr_1k0, self.mmc3.chr_1k4, self.mmc3.chr_1k8, self.mmc3.chr_1kc,
                address,
