...
impl Mapper525 {
    pub fn new() -> Self {
        Self { prg: 0, nt: 0, chr: [0; 8] }
    }
}

impl Mapper for Mapper525 {
    ...
    fn store_prg(&mut self, _cart: &mut Cartridge, address: u16, data: u8) {
        if address >= 0x8000 {
            match address >> 12 {
                0x8 => self.prg = data,
                0x9 => self.nt = data,
                0xB => self.chr[(address & 7) as usize] = data,
                _ => {}
            }
        }
    }
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> {
        let mut state = vec![self.prg, self.nt];
    ...
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        ...
        if p < state.len() { self.nt = state[p]; p += 1; }
        for i in 0..8 {
