...
impl Mapper421 {
    ...
    fn prg_6000_offset(&self, cart: &Cartridge) -> usize {
        ...
        let raw_bank = match prg_mode {
            ...
            1 => (self.prg[3] << 1) | 1,
            2 => self.prg[3],
            3 => Self::reverse_bits(self.prg[3]),
            _ => 0,
        };
...
impl Mapper for Mapper421 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let res = if (address & 0x3FF) == 0 && page_offset != 0x800 {
                self.dip_switches & 0xC0
            } else if (page_offset & 0x800) != 0 {
                match address & 3 {
