...
impl Mapper624 {
    ...
    fn prg_offset(&self, cart: &Cartridge, address: u16) -> usize {
        ...
        let offset = if (self.latch & 0x01) != 0 {
            ((self.latch >> 1) as usize) * 0x4000 + (address as usize & 0x3FFF)
        } else {
            ((self.latch >> 2) as usize) * 0x8000 + (address as usize & 0x7FFF)
        };
        offset % len
    }

    fn chr_offset(&self, address: u16) -> usize {
        ((self.latch >> 1) as usize) * 0x2000 + (address as usize & 0x1FFF)
    }
...
impl Mapper for Mapper624 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        let effective = if (self.latch & 0x02) != 0 {
            (address & 0xFFF0) | (self.dip_switches & 0x0F) as u16
        } else {
            address
        };
        let offset = self.prg_offset(cart, effective);
        FetchResult {
