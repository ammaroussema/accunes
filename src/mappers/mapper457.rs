...
impl Mapper457 {
    ...
    fn prg_slot_bank(&self, slot: u8) -> u16 {
        ...
        let and_mask = if self.reg_latch & 0x08 != 0 { 0x1F } else { 0x0F };
        let or_value = if self.reg_latch & 0x08 != 0 {
            (self.reg_latch & 0xFE) << 4
        } else {
            self.reg_latch << 4
        };
        ((base as u16) & and_mask) | (or_value as u16)
    }
    ...
    fn chr_bank(&self, address: u16) -> u16 {
        ...
        if self.reg_latch & 0x08 != 0 {
            bank as u16
        } else {
            ((bank as u16) & 0x7F) | (((self.reg_latch & 0xFE) as u16) << 7)
        }
    }
}

impl Mapper for Mapper457 {
    fn reset(&mut self) {
        *self = Self::new();
    }
