...
impl Mapper474 {
    ...
    fn prg_bank16(&self) -> u16 {
        ((self.reg >> 3) & 0x1F) as u16 | (((self.reg as u16) << 5) & 0x20)
    }

    fn prg_bank32(&self) -> u16 {
        ((self.reg >> 4) & 0x0F) as u16 | (((self.reg as u16) << 4) & 0x10)
    }

    fn chr_bank(&self, address: u16) -> u16 {
        (self.mmc3.chr_bank(address) as u16) | (((self.reg & 1) as u16) << 8)
    }
}
...
impl Mapper for Mapper474 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let offset = if (self.reg & 4) != 0 {
                (self.prg_bank16() as usize) * 0x4000 + (address as usize & 0x3FFF)
            } else {
                (self.prg_bank32() as usize) * 0x8000 + (address as usize & 0x7FFF)
            };
            return FetchResult {
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if address < 0x6000 && (address & 0x100) != 0 {
            self.reg = data;
        }
