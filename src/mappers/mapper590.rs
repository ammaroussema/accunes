...
impl Mapper for Mapper590 {
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if address >= 0x8000 {
            if !cart.prg_rom.is_empty() {
                self.data = data & cart.prg_rom[address as usize % cart.prg_rom.len()];
            } else {
                self.data = data;
            }
        }
