...
impl Mapper147 {
    ...
    fn bit_swap_write(val: u8) -> u8 {
        (val >> 2) & 0x3F | (val << 6) & 0xC0
    }

    fn bit_swap_read(val: u8) -> u8 {
        (val << 2) & 0xFC | (val >> 6) & 0x03
    }
}
...
impl Mapper for Mapper147 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            let bank = ((self.output >> 4) & 2) | (self.output & 1);
            ...
            if (address & 0x103) == 0x100 {
                ...
                FetchResult { data: Self::bit_swap_read(val), driven: true }
    ...
    fn store_prg(&mut self, _cart: &mut Cartridge, address: u16, data: u8) {
        let swapped = Self::bit_swap_write(data);
        if address >= 0x8000 {
            ...
            match address & 0x103 {
                ...
                0x101 => self.invert = if (swapped & 1) != 0 { 0xFF } else { 0x00 },
                0x102 => { self.staging = swapped & 0x0F; self.inverter = swapped & 0xF0; }
                0x103 => self.increase = (swapped & 1) != 0,
                _ => {}
            }
        }
        self.x = if self.invert != 0 { self.a } else { self.b };
    ...
    fn fetch_ppu(&mut self, _prg_rom: &[u8], chr_rom: &[u8], _prg_ram: &[u8], chr_ram: &[u8], _prg_vram: &[u8], using_chr_ram: bool, nametable_horizontal_mirroring: bool, _alternative_nametable_arrangement: bool, ppu_address_bus: u16, ppu_octal_latch: u8, vram: &[u8]) -> (u8, u16) {
        ...
        if address < 0x2000 {
            let bank = ((self.output >> 1) & 0xF) as usize;
