...
impl Mapper512 {
    ...
    fn mmc3_raw_bank(&self, cart: &Cartridge, page: usize) -> usize {
...
impl Mapper for Mapper512 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let bank = self.mmc3_raw_bank(cart, page) & 0x3F;
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if (0x4000..0x6000).contains(&address) && (address & 0x0100) != 0 {
            self.reg = data & 3;
        }
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            ...
            let byte = if (self.reg & 2) != 0 {
                let bank = chr_bank & 0x03;
            ...
            let byte = if self.reg == 1 {
                let offset = 0x1000 + (address as usize & 0x0FFF);
                if chr_ram.len() >= 0x2000 {
                    chr_ram[offset]
                } else if !prg_vram.is_empty() {
                    prg_vram[(address as usize & 0x0FFF) % prg_vram.len()]
                } else if !vram.is_empty() {
                    vram[(address as usize & 0x07FF) % vram.len()]
                } else {
                    0
                }
            } else {
    ...
    fn store_ppu(&mut self, cart: &mut Cartridge, address: u16, data: u8, vram: &mut [u8]) {
        if address < 0x2000 {
            if (self.reg & 2) != 0 && !cart.chr_ram.is_empty() {
                let chr_bank = mmc3_chr_bank(
                    self.mmc3.r8000,
                    self.mmc3.chr_2k0,
                    self.mmc3.chr_2k8,
            ...
            if self.reg == 1 {
                ...
                } else if !cart.prg_vram.is_empty() {
                    let len = cart.prg_vram.len();
                    cart.prg_vram[(address as usize & 0x0FFF) % len] = data;
                } else {
                    vram[(address as usize & 0x07FF) % vram.len()] = data;
                }
            } else {
                let mirrored = mirror_h_or_v(self.mmc3.nametable_mirroring(), address);
