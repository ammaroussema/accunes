...
impl Mapper322 {
    ...
    fn outer_bank(&self) -> usize {
        (self.reg as usize >> 3 & 3) | (self.reg as usize >> 4 & 4)
    }
}
...
impl Mapper for Mapper322 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if address >= 0x8000 {
            ...
            if (self.reg & 0x20) != 0 {
                let prg_and = if (self.reg & 0x80) != 0 { 0x1F } else { 0x0F };
                let prg_or = outer << 4 & !prg_and;
                ...
                let bank = outer << 3 | (self.reg as usize & 7);
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            let outer = self.outer_bank();
            let chr_and = if (self.reg & 0x80) != 0 { 0xFF } else { 0x7F };
            let chr_or = outer << 7 & !chr_and;
            ...
            let bank = (chr_bank as usize & chr_and) | chr_or;
