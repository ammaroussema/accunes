...
impl Mapper for Mapper533 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            let offset = address as usize & 0x7FFF;
            let mut data = cart.prg_rom[offset % cart.prg_rom.len()];
            if address >= 0xE000 && address < 0xF000 {
                data = (data & 0xF0) | (self.data >> 4);
            }
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if address >= 0x8000 {
            let rom_data = cart.prg_rom[(address as usize) % cart.prg_rom.len()];
    ...
    fn fetch_ppu(&mut self, _prg_rom: &[u8], chr_rom: &[u8], _prg_ram: &[u8], chr_ram: &[u8], _prg_vram: &[u8], using_chr_ram: bool, nametable_horizontal_mirroring: bool, _alternative_nametable_arrangement: bool, ppu_address_bus: u16, ppu_octal_latch: u8, vram: &[u8]) -> (u8, u16) {
        ...
        if address < 0x2000 {
            let bank = ((self.data >> 4) & 1) as usize;
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> { vec![self.data] }
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], start: usize) -> usize { if start < state.len() { self.data = state[start]; start + 1 } else { start } }
