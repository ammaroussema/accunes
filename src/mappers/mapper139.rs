...
impl Mapper for Mapper139 {
    ...
    fn mirror_nametable(&self, _cart: &Cartridge, address: u16) -> u16 {
        match self.reg[7] & 7 {
            0 => {
                if (address & 0x0C00) == 0x0C00 { (address & 0x3FF) | 0x2C00 } else { (address & 0x3FF) | 0x2000 }
            }
            2 => {
    ...
    fn fetch_ppu(&mut self, _prg_rom: &[u8], chr_rom: &[u8], _prg_ram: &[u8], chr_ram: &[u8], _prg_vram: &[u8], using_chr_ram: bool, _nametable_horizontal_mirroring: bool, _alternative_nametable_arrangement: bool, ppu_address_bus: u16, ppu_octal_latch: u8, vram: &[u8]) -> (u8, u16) {
        ...
        if address < 0x2000 {
            let page = (address >> 10) as usize;
            let chrmode = (self.reg[7] & 1) != 0;
            let reg_idx = if chrmode { 0 } else { page >> 1 };
            let base = ((self.reg[reg_idx] as usize) & 0x07) | ((self.reg[4] as usize) << 3);
            let source_page = base * 8 + page;
            let offset = source_page * 0x400 + (address as usize & 0x3FF);
            ...
            let mirrored = match self.reg[7] & 7 {
                0 => if (address & 0x0C00) == 0x0C00 { (address & 0x3FF) | 0x2C00 } else { (address & 0x3FF) | 0x2000 },
                2 => (address & 0x33FF) | ((address & 0x0800) >> 1),
                6 => (address & 0x3FF) | 0x2000,
                _ => address & 0x37FF,
            };
