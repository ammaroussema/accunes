...
impl Mapper539 {
    pub fn new() -> Self { Self { prg: 0, horizontal_mirroring: true, fds: FdsSound::new() } }
    ...
    fn wram_addr(full_addr: u16) -> usize {
        let bank = full_addr >> 12;
        (full_addr as usize & 0x1FFF) | if bank < 0xC { 0x1000 } else { 0 } | if bank < 0x8 { 0x0800 } else { 0 }
    }

    fn is_ram_addr(address: u16) -> bool {
        let high = address >> 8;
        matches!(high, 0x60 | 0x62 | 0x64 | 0x65 | 0x82
...
impl Mapper for Mapper539 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if Self::is_ram_addr(address) {
            let wram_idx = Self::wram_addr(address) % cart.prg_ram.len().max(1);
            return FetchResult { data: cart.prg_ram.get(wram_idx).copied().unwrap_or(0), driven: true };
        }
        match address {
            0x6000..=0x7FFF => FetchResult { data: self.prg_read_bank(0xD, address, &cart.prg_rom), driven: true },
            0x8000..=0x9FFF => FetchResult { data: self.prg_read_bank(0xC, address, &cart.prg_rom), driven: true },
            0xA000..=0xBFFF => FetchResult { data: self.prg_read_bank(self.prg, address, &cart.prg_rom), driven: true },
            0xC000..=0xDFFF => {
                if Self::is_ram_addr(address) {
                    unreachable!()
                }
                FetchResult { data: self.prg_read_bank(0xE, address, &cart.prg_rom), driven: true }
            }
            0xE000..=0xFFFF => FetchResult { data: self.prg_read_bank(0xF, address, &cart.prg_rom), driven: true },
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        ...
        if Self::is_ram_addr(address) {
            let wram_idx = Self::wram_addr(address) % cart.prg_ram.len().max(1);
            if wram_idx < cart.prg_ram.len() { cart.prg_ram[wram_idx] = data; }
            return;
        }
        ...
        } else if address >= 0x5000 && address < 0x6000 && (address & 0xFF) == 0x25 {
            self.horizontal_mirroring = (data & 8) != 0;
        }
    }

    fn mirror_nametable(&self, _cart: &Cartridge, address: u16) -> u16 {
        if self.horizontal_mirroring { (address & 0x3FFF) | ((address & 0x0800) >> 1) } else { address & 0x37FF }
    }
    ...
    fn fetch_ppu(&mut self, _prg_rom: &[u8], chr_rom: &[u8], _prg_ram: &[u8], chr_ram: &[u8], _prg_vram: &[u8], using_chr_ram: bool, _nh: bool, _alt: bool, ppu_addr: u16, ppu_latch: u8, vram: &[u8]) -> (u8, u16) {
        ...
        if address < 0x2000 {
            ...
            let mir = if self.horizontal_mirroring { (address & 0x3FFF) | ((address & 0x0800) >> 1) } else { address & 0x37FF };
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> {
        vec![self.prg, self.horizontal_mirroring as u8]
    }
    ...
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], mut start: usize) -> usize {
        ...
        if start < state.len() { self.horizontal_mirroring = state[start] != 0; start += 1; }
        start
    }

    fn reset(&mut self) {
        self.prg = 0; self.horizontal_mirroring = true; self.fds.reset();
    }
}
