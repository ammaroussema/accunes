...
impl FdsSound {
    ...
    fn run(&mut self, clocks: u32) {
        ...
        if !self.mod_halt {
            ...
            while p < end_pos {
                ...
                if wv == 4 { self.mod_pos = 0; }
                    ...
                    self.mod_pos = (self.mod_pos as i32 + BIAS[(wv & 7) as usize]) as u32 & 0x7F;
                }
                p += 1;
            }
        }
        ...
        if !self.wav_write {
            ...
            self.fout = self.wave[1][idx] * vol_out as i32 - vol_out as i32 * 31;
        }
        const MASTER_SCALE: [f32; 4] = [1.0, 2.0 / 3.0, 0.5, 0.4];
        self.current_audio_sample = (self.fout as f32) / (32.0 * 32.0) * MASTER_SCALE[(self.master_vol & 3) as usize];
    }
    #[allow(dead_code)] fn read_reg(&mut self, adr: u16) -> u8 {
        match adr {
            0x4040..=0x407F => {
                if self.wav_write { self.wave[1][(adr - 0x4040) as usize] as u8 & 0x3F }
                else { self.wave[1][((self.phase[1] >> 16) & 0x3F) as usize] as u8 & 0x3F }
            }
...
impl Mapper535 {
    pub fn new() -> Self {
        Self { prg: 0, irq_enabled: false, irq_counter: 0, irq_acknowledged: false, fds: FdsSound::new() }
    }
}

impl Mapper for Mapper535 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0xB800 && address < 0xD800 {
            let idx = (address as usize - 0xB800) % cart.prg_ram.len().max(1);
            return FetchResult { data: cart.prg_ram.get(idx).copied().unwrap_or(0), driven: true };
        }
        if address >= 0x6000 && address < 0x8000 {
            let bank = self.prg as usize;
            let offset = bank * 0x2000 + (address as usize & 0x1FFF);
            let prg_rom = &cart.prg_rom;
            return FetchResult { data: if prg_rom.is_empty() { 0xFF } else { prg_rom[offset % prg_rom.len()] }, driven: true };
        }
        if address >= 0x8000 {
            let offset = 3 * 0x8000 + (address as usize & 0x7FFF);
            let prg_rom = &cart.prg_rom;
            return FetchResult { data: if prg_rom.is_empty() { 0xFF } else { prg_rom[offset % prg_rom.len()] }, driven: true };
        }
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        ...
        if address >= 0xB800 && address < 0xD800 {
            let idx = (address as usize - 0xB800) % cart.prg_ram.len().max(1);
            if idx < cart.prg_ram.len() { cart.prg_ram[idx] = data; }
            return;
        }
    ...
    fn fetch_ppu(&mut self, _prg_rom: &[u8], chr_rom: &[u8], _prg_ram: &[u8], chr_ram: &[u8], _prg_vram: &[u8], using_chr_ram: bool, nametable_horizontal_mirroring: bool, _alternative_nametable_arrangement: bool, ppu_address_bus: u16, ppu_octal_latch: u8, vram: &[u8]) -> (u8, u16) {
        ...
        if address < 0x2000 {
            ...
            if using_chr_ram { new_addr_bus |= chr_ram.get(offset).copied().unwrap_or(0) as u16; }
            else { new_addr_bus |= chr_rom.get(offset % chr_rom.len().max(1)).copied().unwrap_or(0) as u16; }
        } else if address >= 0x2000 && address < 0x3F00 {
            let mirrored = if nametable_horizontal_mirroring { (address & 0x3FFF) | ((address & 0x0800) >> 1) } else { address & 0x37FF };
            new_addr_bus |= vram.get((mirrored & 0x7FF) as usize).copied().unwrap_or(0) as u16;
        }
    ...
    fn cpu_clock(&mut self, cycles: u8) -> bool {
        if self.irq_enabled {
            ...
            if self.irq_counter >= 7560 {
                self.irq_counter -= 7560;
                return true;
            }
        }
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> {
        let mut state = vec![self.prg, self.irq_enabled as u8];
    ...
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], mut start: usize) -> usize {
        if start < state.len() { self.prg = state[start]; start += 1; }
        if start < state.len() { self.irq_enabled = state[start] != 0; start += 1; }
        if start + 2 <= state.len() { self.irq_counter = u16::from_le_bytes([state[start], state[start + 1]]); start += 2; }
        start
    }

    fn reset(&mut self) {
        self.prg = 0; self.irq_enabled = false; self.irq_counter = 0; self.irq_acknowledged = false;
        self.fds.reset();
    }
}
