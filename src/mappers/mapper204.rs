...
impl Mapper for Mapper204 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if (self.addr & 0xFF0F) == 0xF004 || (self.addr & 0xFF0F) == 0xF008 {
            return FetchResult { data: 0, driven: false };
        }
        ...
        let offset = if self.addr & 0x20 != 0 {
            let num_32k = (prg_len / 0x8000).max(1);
            let bank = ((self.addr >> 1) as usize) % num_32k;
            bank * 0x8000 + (address as usize & 0x7FFF)
        } else {
            let num_16k = (prg_len / 0x4000).max(1);
            let bank = (self.addr as usize) % num_16k;
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            let bank = if self.addr & 0x20 != 0 {
                (self.addr & 0x0E) as usize
            } else {
                (self.addr & 0x0F) as usize
            };
            ...
            let mirrored = if self.addr & 0x10 != 0 {
                (address & 0x33FF) | ((address & 0x0800) >> 1)
            } else {
                address & 0x37FF
            };
