...
impl Mapper615 {
    ...
    fn prg_16k_banks(&self) -> (usize, usize) {
        let prg = (self.reg[0] as usize & 0x34) | (((self.reg[1] as usize) << 3) & 0x08);
        if (self.reg[0] & 0x08) != 0 {
            if (self.reg[0] & 0x20) != 0 && (self.reg[1] & 0x01) != 0 {
                let bank0 = (prg & 0x3C) | (self.reg[2] as usize & 0x03);
                let bank1 = (prg & 0x3C) | 0x03;
                (bank0, bank1)
            } else {
                let bank0 = (prg & 0x38) | (self.reg[2] as usize & 0x07);
                let bank1 = (prg & 0x38) | 0x07;
                (bank0, bank1)
            }
        } else {
            let bank32 = ((prg >> 1) & 0x1C) | (((self.reg[0] as usize) >> 1) & 0x03);
            (bank32 * 2, bank32 * 2 + 1)
        }
    }
}

impl Mapper for Mapper615 {
    fn reset(&mut self) {
        self.reg = [0; 3];
    }
    ...
    fn mirror_nametable(&self, _cart: &Cartridge, address: u16) -> u16 {
        mirror_h_or_v((self.reg[0] & 0x80) != 0, address)
    }
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            ...
            let mirrored = mirror_h_or_v((self.reg[0] & 0x80) != 0, address);
