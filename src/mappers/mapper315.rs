...
impl Mapper for Mapper315 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let reg_base = ((self.reg as usize) << 3) & !0x0F;
            let invert = (self.mmc3.r8000 & 0x40) != 0;
            let fixed_second_last = num_8k.saturating_sub(2);
            let fixed_last = num_8k.saturating_sub(1);
            let raw_bank = match address {
                0xE000..=0xFFFF => fixed_last,
                0xC000..=0xDFFF => {
                    if invert { self.mmc3.bank_8c as usize }
                    else { fixed_second_last }
                }
                0xA000..=0xBFFF => self.mmc3.bank_a as usize,
                0x8000..=0x9FFF => {
                    if invert { fixed_second_last }
                    else { self.mmc3.bank_8c as usize }
                }
                _ => 0,
            };
            if (self.reg & 8) != 0 {
                let bank = (raw_bank & !3) | (address as usize >> 13 & 3);
                ...
                let bank = (raw_bank & 0x0F) | reg_base;
                let bank = bank % num_8k;
                let offset = bank * 0x2000 + (address as usize & 0x1FFF);
                FetchResult {
                    data: cart.prg_rom[offset % cart.prg_rom.len().max(1)],
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            let base = ((self.reg as usize) << 3 & 0x040)
                | ((self.reg as usize) << 6 & 0x080)
                | ((self.reg as usize) << 8 & 0x100);
            ...
            let bank = ((bank_in as usize) & 0xFF) | base;
