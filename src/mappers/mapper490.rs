...
impl Mapper490 {
    ...
    fn chr_base(&self) -> u16 {
        (self.reg as u16 & 0x10) << 4
    }
}

impl Mapper for Mapper490 {
    fn reset(&mut self) {
        self.reg = 0;
        self.mmc3.reset();
    }
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let effective = if (self.reg & 0x80) != 0 {
                (address & 0xFFF0) | (self.mmc3.get_dip_switches() & 0xF) as u16
            } else {
                address
            };
            let offset = if (self.reg & 0x20) != 0 {
                ((self.reg >> 1) as usize) * 0x8000 + (effective as usize & 0x7FFF)
            } else {
                (self.reg as usize) * 0x4000 + (effective as usize & 0x3FFF)
            };
            return FetchResult {
    ...
    fn fetch_ppu(
        ...
        let bank = (self.mmc3.chr_bank(address) as u16) | self.chr_base();
