...
impl Mapper607 {
    ...
    fn prg_16k_banks(&self) -> (usize, usize) {
        if (self.reg & 0x10) != 0 {
            let high = ((self.reg as usize) << 3) & !0x07;
            let bank0 = high | (self.latch as usize & 0x07);
            let bank1 = high | 0x07;
            (bank0, bank1)
        } else {
            let bank32 = (((self.reg as usize) << 3) & 0x04) | (((self.latch as usize) >> 1) & 0x03);
            (bank32 * 2, bank32 * 2 + 1)
        }
    }
}

impl Mapper for Mapper607 {
    fn reset(&mut self) {
        self.reg = 0;
        self.latch = 0;
    }
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            ...
            let mirrored = mirror_h_or_v((self.reg & 0x20) != 0, address);
