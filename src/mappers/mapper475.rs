...
impl Mapper475 {
    ...
    fn chr_bank(&self, address: u16) -> u16 {
        ...
        let chr_or = if self.reset_state {
            ((self.reg as u16) << 7) & 0x180
        } else {
            0x200
        };
        (raw & 0x7F) | chr_or
...
impl Mapper for Mapper475 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let offset = if self.reset_state {
                if (self.reg & 3) == 3 {
                    let bank32 =
                        (((self.reg as u16) & 0x0C) << 2) | (((self.reg as u16) & 0x0C) >> 2);
                    (bank32 as usize) * 0x8000 + (address as usize & 0x7FFF)
                } else {
                    let cpu_bank = ((address - 0x8000) / 0x2000) as u8;
                    let raw = self.prg_raw_bank(cart, cpu_bank);
                    let bank8 = (raw & 0x0F) | (self.reg & 0xF0);
                    (bank8 as usize) * 0x2000 + (address as usize & 0x1FFF)
                }
            } else {
                let bank32 = (((self.reg >> 4) & 0x3) as u16) | 0x10;
                (bank32 as usize) * 0x8000 + (address as usize & 0x7FFF)
            };
            return FetchResult {
