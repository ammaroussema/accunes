...
impl Mapper37 {
    ...
    fn remap_prg(&self, raw: u8) -> usize {
        ...
        if block <= 2 {
            ...
            ((raw & 0x07) | 0x08) as usize
        } else if block == 7 {
            ((raw & 0x07) | 0x20) as usize
        } else {
            ((raw & 0x0F) | 0x10) as usize
        }
    }

    fn remap_chr(&self, raw: u8) -> usize {
        let mut page = raw as usize;
...
impl Mapper for Mapper37 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let bank = match address {
                ...
                0xC000..=0xDFFF => {
                    if invert {
                        self.remap_prg(self.raw_prg_0) % num_8k
                    } else {
                ...
                0xA000..=0xBFFF => {
                    self.remap_prg(self.raw_prg_1) % num_8k
                }
                _ => {
                    if invert {
                        let raw_fixed = num_8k.saturating_sub(2) as u8;
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        ...
        if address >= 0x8000 {
            if (address & 0xE001) == 0x8001 {
                ...
                match mode {
                    6 => self.raw_prg_0 = data,
                    7 => self.raw_prg_1 = data,
                    _ => {}
                }
            }
            self.mmc3.store_prg(cart, address, data);
    ...
    fn save_mapper_registers(&self, cart: &Cartridge) -> Vec<u8> {
        ...
        state.push(self.raw_prg_0);
        state.push(self.raw_prg_1);
        state
    }
    ...
    fn load_mapper_registers(&mut self, cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        ...
        if idx < state.len() { self.raw_prg_0 = state[idx]; idx += 1; }
        if idx < state.len() { self.raw_prg_1 = state[idx]; idx += 1; }
        idx
    }
}
