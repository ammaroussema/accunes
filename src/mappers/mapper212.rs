...
impl Mapper for Mapper212 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if (self.addr & 0x4000) != 0 {
            let bank = ((self.addr >> 1) & 3) as usize;
            let num_32k = (prg_len / 0x8000).max(1);
            let offset = (bank % num_32k) * 0x8000 + (address as usize & 0x7FFF);
            FetchResult { data: cart.prg_rom[offset % prg_len], driven: true }
        } else {
            let bank = (self.addr & 7) as usize;
            let num_16k = (prg_len / 0x4000).max(1);
            let offset = (bank % num_16k) * 0x4000 + (address as usize & 0x3FFF);
            FetchResult { data: cart.prg_rom[offset % prg_len], driven: true }
        }
    }
