...
impl Mapper503 {
    ...
    fn chr_bank(&self, address: u16) -> usize {
        let mmc3_bank = (self.mmc3.chr_bank(address) as usize) & 0x7F;
        let outer = ((self.reg as usize) << 4) & !0x7F;
        mmc3_bank | outer
    }
}

impl Mapper for Mapper503 {
    fn reset(&mut self) {
        self.reg = 0;
        self.irq_ack = false;
        self.mmc3.reset();
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let effective = if (self.reg & 0x20) != 0 {
                (address & !0x03) | (self.mmc3.get_dip_switches() as u16 & 0x03)
            } else {
                address
            };
            let offset = if (self.reg & 0x10) != 0 {
                let bank = (self.reg >> 1) as usize;
                bank * 0x8000 + (effective as usize & 0x7FFF)
            } else {
                let bank = self.reg as usize;
                bank * 0x4000 + (effective as usize & 0x3FFF)
            };
            return FetchResult {
