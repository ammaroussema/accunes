...
impl Mapper444 {
    ...
    fn prg_and(&self) -> u8 {
        if (self.submapper & 4) != 0 && (self.reg & 0x02) != 0 {
            0x1F
        } else {
            0x0F
        }
    ...
    fn prg_or_resolved(&self) -> u8 {
        (self.reg << 4) & !self.prg_and()
    }

    fn chr_a17_mmc3(&self) -> bool {
        (self.submapper & 1) != 0
    }

    fn chr_a18_d4(&self) -> bool {
        (self.submapper & 2) != 0
    }

    fn chr_and(&self) -> u16 {
        if self.chr_a17_mmc3() {
            0x00FF
        } else {
            0x007F
        }
    ...
    fn chr_or(&self) -> u16 {
        let mut chr_or = if self.chr_a17_mmc3() {
            0
        } else {
            ((self.reg & 0x01) as u16) << 7
        };
        if self.chr_a18_d4() {
            chr_or |= ((self.reg & 0x10) as u16) << 4;
        } else {
            chr_or |= ((self.reg & 0x02) as u16) << 7;
        }
        chr_or
    }
    ...
    fn get_prg_page(&self, cart: &Cartridge, cpu_bank: u8) -> u8 {
        if (self.reg & 0x04) != 0 {
            let mask = if (self.reg & 0x08) != 0 { 1 } else { 3 };
    ...
    fn prg_bank8(&self, cart: &Cartridge, cpu_bank: u8) -> u8 {
        (self.get_prg_page(cart, cpu_bank) & self.prg_and()) | self.prg_or_resolved()
    }
...
impl Mapper for Mapper444 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            if (self.reg & 0x08) != 0 && (self.reg & 0x04) == 0 {
                return FetchResult {
