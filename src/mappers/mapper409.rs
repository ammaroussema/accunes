...
pub struct Mapper409 {
    addr: u16,
}

impl Mapper409 {
    pub fn new() -> Self { Self { addr: 0 } }
}

impl Mapper for Mapper409 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            if address < 0xC000 {
                let offset = (self.addr as usize) * 0x4000 + (address as usize & 0x3FFF);
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if address >= 0x6000 && address < 0x8000 {
            let off = (address - 0x6000) as usize;
            if off < cart.prg_ram.len() { cart.prg_ram[off] = data; }
        } else if address >= 0x8000 {
            self.addr = (address & 0x0FFF) as u16;
        }
    }
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> { self.addr.to_le_bytes().to_vec() }
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], start: usize) -> usize { if start + 2 <= state.len() { self.addr = u16::from_le_bytes([state[start], state[start + 1]]); start + 2 } else { start } }
    fn reset(&mut self) { self.addr = 0; }
}
