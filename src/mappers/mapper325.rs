...
fn prg_remap(bank: usize) -> usize {
    bank & !0x0C | (bank >> 1 & 4) | (bank << 1 & 8)
}

fn chr_remap(bank: usize) -> usize {
    bank & !0x22 | (bank >> 4 & 2) | (bank << 4 & 0x20)
}
...
impl Mapper for Mapper325 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if address >= 0x8000 {
            ...
            let bank_mod = prg_remap(mmc3_bank);
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if address >= 0x8000 {
            let mapped = (address & 0xE000) | ((address >> 3) & 1);
            self.mmc3.store_prg(cart, mapped, data);
        } else {
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            ...
            let bank = chr_remap(chr_bank as usize);
