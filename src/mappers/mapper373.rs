...
impl Mapper373 {
    ...
    fn get_raw_mmc3_prg_bank(&self, slot: usize) -> u8 {
    ...
    fn get_effective_prg_bank(&self, slot: usize) -> usize {
        let raw = if self.gnrom_mode() {
            let base = self.get_raw_mmc3_prg_bank(slot & 1) as usize;
            (base & !3) | (slot & 3)
        } else {
            self.get_raw_mmc3_prg_bank(slot) as usize
        };
        self.remap_prg(raw)
    }
...
impl Mapper for Mapper373 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let bank = self.get_effective_prg_bank(slot) % num_8k;
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            ...
            let byte = if chr_rom.is_empty() { 0 } else { chr_rom[offset % chr_rom.len()] };
