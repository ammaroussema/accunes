...
impl Mapper571 {
    ...
    fn sync(&mut self) {
        let prg_and: u16 = if self.reg & 0x10 != 0 { 0x1F } else { 0x0F };
        let chr_and: u16 = if self.reg & 0x10 != 0 { 0xFF } else { 0x7F };
        let prg_or: u16 = (self.reg as u16) << 1;
        let chr_or: u16 = (self.reg as u16) << 4;
        self.a0 = if self.reg & 0x10 != 0 { 8 } else { 4 };
        self.a1 = if self.reg & 0x10 != 0 { 4 } else { 8 };
        let _ = (prg_and, prg_or, chr_and, chr_or);
    }

    fn chr_page(&self, bank: usize) -> u16 {
        let chr_and: u16 = if self.reg & 0x10 != 0 { 0xFF } else { 0x7F };
        let chr_or: u16 = (self.reg as u16) << 4;
        ((self.chr[bank] & chr_and) | (chr_or & !chr_and)) as u16
    }

    fn prg_page(&self, address: u16) -> u16 {
        if self.reg & 0x20 != 0 {
            let prg_and: u16 = if self.reg & 0x10 != 0 { 0x1F } else { 0x0F };
            let prg_or: u16 = (self.reg as u16) << 1;
            let or_masked = prg_or & !prg_and;
            let raw = match address {
                0x8000..=0x9FFF => self.prg[0] as u16,
                0xA000..=0xBFFF => self.prg[1] as u16,
                0xC000..=0xDFFF => 0xFE,
                _ => 0xFF,
            };
            (raw & prg_and) | or_masked
        } else if self.reg & 0x06 != 0 {
            let bank = (self.reg >> 1) as u16;
            match address {
                0x8000..=0x9FFF => bank * 4,
                0xA000..=0xBFFF => bank * 4 + 1,
                0xC000..=0xDFFF => bank * 4 + 2,
                _ => bank * 4 + 3,
            }
        } else {
            let bank = (self.reg as u16) * 2;
            match address {
                0x8000..=0x9FFF => bank,
                0xA000..=0xBFFF => bank + 1,
                0xC000..=0xDFFF => bank,
                _ => bank + 1,
            }
        }
...
impl Mapper for Mapper571 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if address >= 0x8000 {
            ...
            let offset = (page as usize * 0x2000 + (address as usize & 0x1FFF)) % len;
            return FetchResult {
    ...
    fn store_prg(&mut self, _cart: &mut Cartridge, address: u16, data: u8) {
        ...
        match address {
            ...
            0x9000..=0x9FFF => {
                let reg = if (address & self.a1) != 0 { 2 } else { 0 }
                    | if (address & self.a0) != 0 { 1 } else { 0 };
                match reg & 3 {
                    0 | 1 => {
            ...
            0xB000..=0xEFFF => {
                ...
                let reg = ((bank - 0xB) << 1) | if (address & self.a1) != 0 { 1 } else { 0 };
                if (address & self.a0) != 0 {
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> {
        ...
        s.extend_from_slice(&self.a0.to_le_bytes());
        s.extend_from_slice(&self.a1.to_le_bytes());
        s.push(self.irq);
        s.push(self.counter);
        s.push(self.latch);
    ...
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        ...
        if p + 2 <= state.len() {
            self.a0 = u16::from_le_bytes([state[p], state[p + 1]]);
            p += 2;
        }
        if p + 2 <= state.len() {
            self.a1 = u16::from_le_bytes([state[p], state[p + 1]]);
            p += 2;
        }
        if p < state.len() {
            self.irq = state[p];
