...
impl Mapper568 {
    ...
    fn second_last_prg_bank(&self, cart: &Cartridge) -> u16 {
        let num = cart.prg_rom.len() / 0x2000;
        if num < 2 { 0 } else { (num - 2) as u16 }
    }

    fn last_prg_bank(&self, cart: &Cartridge) -> u16 {
        let num = cart.prg_rom.len() / 0x2000;
    ...
    fn prg_raw_bank(&self, cart: &Cartridge, bank8: usize) -> u16 {
        if (self.reg & 0x02) != 0 {
            let mask = if (self.reg & 0x01) != 0 { 3 } else { 1 };
            (self.std_prg_bank(cart, 0) & !mask) | ((bank8 as u16) & mask)
        } else {
    ...
    fn prg_and(&self) -> u16 {
        if (self.reg & 0x20) != 0 { 0x1F } else { 0x0F }
    }

    fn chr_and(&self) -> u16 {
        if (self.reg & 0x20) != 0 { 0xFF } else { 0x7F }
    }
    ...
    fn chr_page(&self, address: u16) -> u16 {
        ...
        let chr_or = (self.reg as u16) << 4;
        (raw & self.chr_and()) | (chr_or & !self.chr_and())
    }
}

impl Mapper for Mapper568 {
    fn reset(&mut self) {
        self.reg = 0;
        self.mmc3.reset();
    }
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        let read_addr = if (self.reg & 0x40) != 0 {
            (address & 0xFFF0) | ((self.dip_switches as u16) & 0x0F)
        } else {
            address
        };
        let bank8 = (read_addr as usize - 0x8000) / 0x2000;
        if bank8 > 3 {
            return FetchResult { data: 0, driven: false };
        }
        let raw = self.prg_raw_bank(cart, bank8);
        let prg_or = (self.reg as u16) << 1;
        let page = (raw & self.prg_and()) | (prg_or & !self.prg_and());
        let offset = (page as usize) * 0x2000 + (read_addr as usize & 0x1FFF);
