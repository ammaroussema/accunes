...
impl Mapper for Mapper549 {
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        match address {
            0x6000..=0x7FFF => {
                let bank = ((self.latch_addr >> 2) | if self.latch_addr & 0x20 != 0 { 0x04 } else { 0x00 }) as usize;
                let offset = bank * 0x2000 + (address as usize & 0x1FFF);
                FetchResult { data: cart.prg_rom[offset % cart.prg_rom.len().max(1)], driven: true }
            }
            0x8000..=0xFFFF => {
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        ...
        if address >= 0x8000 {
            let rom_data = cart.prg_rom.get(address as usize % cart.prg_rom.len().max(1)).copied().unwrap_or(0);
            self.latch_addr = address;
            self.latch_data = data & rom_data;
        }
    }
    ...
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], mut start: usize) -> usize {
        if start + 2 <= state.len() { self.latch_addr = u16::from_le_bytes([state[start], state[start + 1]]); start += 2; }
