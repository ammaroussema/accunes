...
impl Mapper596 {
    ...
    fn prg_and(&self) -> usize {
        if (self.reg & 0x04) != 0 { 0x1F } else { 0x0F }
    }

    fn prg_or(&self) -> usize {
        let and = self.prg_and();
        ((self.reg as usize) << 4) & !and
    }

    fn chr_or(&self) -> usize {
        ((self.reg as usize) << 7) & 0x80
    }
    ...
    fn mmc3_raw_prg_bank(&self, cart: &Cartridge, slot: usize) -> usize {
    ...
    fn prg_page(&self, cart: &Cartridge, address: u16) -> usize {
        ...
        let raw = self.mmc3_raw_prg_bank(cart, slot);
    ...
    fn chr_page(&self, address: u16) -> usize {
        ...
        (raw & 0x7F) | self.chr_or()
    }
}

impl Mapper for Mapper596 {
    fn reset(&mut self) {
        self.reg = 0;
        self.mmc3.reset();
    }
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if address >= 0xC000 && (self.reg & 0x08) == 0 {
            self.reg = data;
        }
