...
impl UxromConfig {
    pub fn for_ines(header: &[u8], sub_mapper: u8, chr_size: u8) -> Self {
...
fn prg_banks(cart: &Cartridge) -> usize {
    cart.prg_rom.len() / 0x4000
}

fn prg16_bank_index(cart: &Cartridge, bank: u8) -> usize {
    let banks = prg_banks(cart);
    if banks == 0 {
        0
    } else {
        (bank as usize) % banks
...
fn fixed_bank_index(cart: &Cartridge) -> usize {
    let banks = prg_banks(cart);
    if banks == 0 {
        0
    } else {
        (cart.prg_size_minus_1 as usize) % banks
    }
}
pub(crate) fn read_prg_rom(cart: &Cartridge, bank_select: u8, address: u16) -> u8 {
    let len = cart.prg_rom.len();
    if len == 0 {
        return 0;
    }
    if address >= 0xC000 {
        let bank = fixed_bank_index(cart);
        let offset = bank * 0x4000 + (address as usize & 0x3FFF);
        cart.prg_rom[offset % len]
    } else if address >= 0x8000 {
        let bank = prg16_bank_index(cart, bank_select);
        let offset = bank * 0x4000 + (address as usize & 0x3FFF);
        cart.prg_rom[offset % len]
    } else {
        0
...
fn prg_ram_index(config: &UxromConfig, address: u16) -> Option<usize> {
...
impl Mapper for MapperUxROM {
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if let Some(idx) = prg_ram_index(&self.config, address) {
            ...
            self.bank_select = if self.config.bus_conflict {
                data & read_prg_rom(cart, self.bank_select, address)
            } else {
                data
            };
