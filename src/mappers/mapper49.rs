...
impl Mapper49 {
    ...
    fn remap_chr(&self, raw: usize) -> usize {
        (raw & 0x7F) | (0x80 * self.selected_block as usize)
    }

    fn remap_prg_mmc3(&self, raw: usize) -> usize {
        (raw & 0x0F) | (0x10 * self.selected_block as usize)
    }
...
impl Mapper for Mapper49 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let bank = if !self.prg_mode {
                let base = self.prg_reg as usize * 4;
                ...
                let raw = match address {
                    0xE000..=0xFFFF => {
                        self.remap_prg_mmc3(num_8k.saturating_sub(1))
                    }
                    0xC000..=0xDFFF => {
                        if invert {
                            self.remap_prg_mmc3(self.mmc3.bank_8c as usize)
                        } else {
                            self.remap_prg_mmc3(num_8k.saturating_sub(2))
                        }
                    }
                    0xA000..=0xBFFF => {
                        self.remap_prg_mmc3(self.mmc3.bank_a as usize)
                    }
                    _ => {
                        if invert {
                            self.remap_prg_mmc3(num_8k.saturating_sub(2))
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if address >= 0x6000 && address < 0x8000 {
            if (self.mmc3.prg_ram_protect & 0x40) == 0 {
                self.selected_block = (data >> 6) & 0x03;
                self.prg_reg       = (data >> 4) & 0x03;
                self.prg_mode      = (data & 0x01) != 0;
            }
        } else {
    ...
    fn save_mapper_registers(&self, cart: &Cartridge) -> Vec<u8> {
        ...
        state.push(if self.prg_mode { 1 } else { 0 });
        state
    }

    fn load_mapper_registers(&mut self, cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        let mut idx = self.mmc3.load_mapper_registers(cart, state, start);
        if idx < state.len() { self.selected_block = state[idx]; idx += 1; }
        if idx < state.len() { self.prg_reg        = state[idx]; idx += 1; }
        if idx < state.len() { self.prg_mode       = state[idx] != 0; idx += 1; }
        idx
    }
}
