...
impl Mapper467 {
    ...
    fn sync(&mut self) {
        self.mmc3.r8000 &= 0x3F;
        self.mmc3.set_nametable_horizontal((self.reg & 0x80) != 0);
    }

    fn prg_bank8(&self, slot: usize) -> u16 {
        if self.reg & 0x20 != 0 {
            let prg_and: u16 = if self.reg & 0x40 != 0 { 0x0F } else { 0x03 };
            let prg_or: u16 = (self.reg as u16) << 1;
            let base = match slot {
                0 => self.mmc3.bank_8c as u16,
                1 => self.mmc3.bank_a as u16,
                2 => 0xFE,
                _ => 0xFF,
            };
            (base & prg_and) | (prg_or & !prg_and)
        } else {
            let bank16 = (self.reg as u16) & 0x1F;
            bank16 * 2 + (slot as u16 & 1)
        }
    }

    fn chr_bank(&self, address: u16) -> u16 {
        let slot = ((address >> 10) & 7) as usize;
        let pair = slot >> 1;
        let inner = match pair {
            0 => self.mmc3.chr_2k0 as u16,
            1 => (self.mmc3.chr_2k0 | 1) as u16,
            2 => self.mmc3.chr_1k0 as u16,
            _ => self.mmc3.chr_1k4 as u16,
        };
        let high = ((self.reg as u16) << 2) & 0x100;
        let v = if self.reg & 0x40 != 0 {
            inner | high
        } else {
            (inner & !3) | pair as u16 | high
        };
        (v << 1) | (slot as u16 & 1)
    }
}

impl Mapper for Mapper467 {
    fn reset(&mut self) {
        self.reg = 0;
        self.mmc3.reset();
        self.sync();
