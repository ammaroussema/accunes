...
impl Mapper for Mapper392 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            if (self.reg & 0x10) == 0 {
                let offset = 0x20usize * 0x8000 + (address as usize & 0x7FFF);
                FetchResult {
                ...
                let raw_bank = self.prg_raw_bank(cart, cpu_bank);
                let bank = ((raw_bank as u16) & 0x0F) | ((self.reg as u16) << 4);
    ...
    fn fetch_ppu(
        ...
        if (self.reg & 0x10) != 0 {
            ...
            let bank = ((raw_bank as u16) & 0x7F) | ((self.reg as u16) << 7);
