...
impl Mapper626 {
    ...
    fn prg_offset(&self, cart: &Cartridge, address: u16) -> usize {
        ...
        let offset = if (self.latch & 0x200) != 0 {
            (self.latch as usize) * 0x8000 + (address as usize & 0x7FFF)
        } else {
            let bank = ((self.latch << 1) | ((self.latch >> 8) & 1)) as usize;
