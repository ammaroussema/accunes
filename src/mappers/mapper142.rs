...
impl Mapper for Mapper142 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        let banks = self.sync_prg();
        let offset = if address >= 0xE000 {
            self.prg_addr(cart, banks[3], address)
        } else if address >= 0xC000 {
            self.prg_addr(cart, banks[2], address)
        } else if address >= 0xA000 {
            self.prg_addr(cart, banks[1], address)
        } else if address >= 0x8000 {
            self.prg_addr(cart, banks[0], address)
        } else {
            if self.rom6 {
                self.prg_addr(cart, self.prg[3] & 0x0F, address)
            } else {
                let ram_offset = (address - 0x6000) as usize;
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        match address {
            0x8000..=0xBFFF => {
                let nibble = ((address >> 12) & 7) << 2;
                let mask = !(0x0F << nibble);
                let val = (data & 0x0F) as u16;
                self.irq_latch = (self.irq_latch & mask) | (val << nibble);
            }
            0xC000..=0xCFFF => {
            ...
            0xE000..=0xEFFF => {
                self.prg_bank_index = data & 0x07;
            }
            0xF000..=0xFFFF => {
                if self.prg_bank_index >= 1 && self.prg_bank_index <= 4 {
                    self.prg[(self.prg_bank_index - 1) as usize] = data;
                } else if self.prg_bank_index == 5 {
                    self.rom6 = (data & 0x04) != 0;
                }
            }
            _ => {}
        }
        ...
        if address >= 0x6000 && address < 0x8000 && !self.rom6 {
            let ram_offset = (address - 0x6000) as usize;
            if ram_offset < cart.prg_ram.len() {
    ...
    fn cpu_clock(&mut self, cycles: u8) -> bool {
        ...
        if (self.irq_control & 2) != 0 {
            ...
            self.irq_counter = self.irq_counter.wrapping_add(cycles as u16);
            if self.irq_counter < prev {
                self.irq_counter = self.irq_latch;
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> {
        ...
        state.push(self.prg_bank_index);
        for b in &self.prg {
            state.push(*b);
        }
        ...
        state.push(if self.rom6 { 1 } else { 0 });
    ...
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        let mut p = start;
        if p >= state.len() { return start; }
        self.prg_bank_index = state[p]; p += 1;
        for b in &mut self.prg {
            if p >= state.len() { return start; }
            *b = state[p]; p += 1;
        }
        if p >= state.len() { return start; }
        self.irq_control = state[p]; p += 1;
        if p + 2 <= state.len() {
            self.irq_counter = u16::from_le_bytes([state[p], state[p + 1]]); p += 2;
        } else { return start; }
        if p + 2 <= state.len() {
            self.irq_latch = u16::from_le_bytes([state[p], state[p + 1]]); p += 2;
        } else { return start; }
        if p >= state.len() { return start; }
        self.rom6 = state[p] != 0; p += 1;
        if p >= state.len() { return start; }
        self.irq_pending = state[p] != 0; p += 1;
        if p >= state.len() { return start; }
        self.master_io = state[p] != 0; p += 1;
        if p >= state.len() { return start; }
        self.master_vol = state[p]; p += 1;
        for t in 0..2 {
            for w in &mut self.wave[t] {
                if p + 4 <= state.len() {
                    *w = i32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
                } else { return start; }
            }
        }
        for f in &mut self.freq {
            if p + 4 <= state.len() {
                *f = u32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
            } else { return start; }
        }
        for ph in &mut self.phase {
            if p + 4 <= state.len() {
                *ph = u32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
            } else { return start; }
        }
        if p >= state.len() { return start; }
        self.wav_write = state[p] != 0; p += 1;
        if p >= state.len() { return start; }
        self.wav_halt = state[p] != 0; p += 1;
        if p >= state.len() { return start; }
        self.env_halt = state[p] != 0; p += 1;
        if p >= state.len() { return start; }
        self.mod_halt = state[p] != 0; p += 1;
        if p + 4 <= state.len() {
            self.mod_pos = u32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
        } else { return start; }
        if p + 4 <= state.len() {
            self.mod_write_pos = u32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
        } else { return start; }
        ...
        for et in &mut self.env_timer {
            if p + 4 <= state.len() {
                *et = u32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
            } else { return start; }
        }
        for es in &mut self.env_speed {
            if p + 4 <= state.len() {
                *es = u32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
            } else { return start; }
        }
        for eo in &mut self.env_out {
            if p + 4 <= state.len() {
                *eo = u32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
            } else { return start; }
        }
        if p + 4 <= state.len() {
            self.master_env_speed = u32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
        } else { return start; }
        if p + 4 <= state.len() {
            self.rc_accum = i32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
        } else { return start; }
        if p + 4 <= state.len() {
            self.fout = i32::from_le_bytes([state[p], state[p + 1], state[p + 2], state[p + 3]]); p += 4;
        }
        p
    }
}
