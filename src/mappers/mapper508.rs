...
impl Mapper for Mapper508 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let offset = if (self.reg & 0x80) != 0 {
                let page = (address as usize - 0x8000) / 0x2000;
                let prg_and = 0x1F;
                let prg_or = ((self.reg as usize) >> 1) & 0x20;
                let bank = (self.mmc3_raw_bank(cart, page) & prg_and) | prg_or;
                bank * 0x2000 + (address as usize & 0x1FFF)
            } else {
                let outer_16k = (self.reg as usize & 0x0F) | (((self.reg as usize) >> 2) & 0x10);
                if (self.reg & 0x20) != 0 {
                    let bank32 = outer_16k >> 1;
                    bank32 * 0x8000 + (address as usize & 0x7FFF)
                } else {
                    let bank16 = outer_16k;
