...
impl Mapper344 {
    pub fn new(header: &[u8], _rom: &[u8], _rom_name: &str) -> Self {
        ...
        Self { mmc3: MapperMMC3::new(config), reg: 0, dip_switches: 0 }
    }
}
...
impl Mapper for Mapper344 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        if address >= 0x8000 {
            ...
            let mmc3_bank = if (self.reg & 0x04) != 0 {
                let bank0 = if mode { second_last } else { self.mmc3.bank_8c as usize };
                (bank0 & !3) | bank
            } else {
                match (bank, mode) {
            ...
            let outer = (self.reg as usize) << 4 & 0xF0;
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            ...
            let outer = (self.reg as usize) << 7 & !chr_and & 0xFF;
            let bank = (chr_bank as usize & chr_and) | outer;
