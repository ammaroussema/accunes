...
impl Mapper for Mapper530 {
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if (0x6000..0x8000).contains(&address) {
            ...
            let raw_bank = (address >> 12) & 0x0F;
            let bank = raw_bank | ((address & 0x08) >> 3);

            let bit0 = (address & 0x01) != 0;
            let bit1 = (address & 0x02) != 0;
            let decoded_reg = (if bit1 { 2 } else { 0 }) | (if bit0 { 1 } else { 0 });

            match bank {
                0x8 => {
                    let val = ((data & 0x02) << 2) | ((data & 0x08) >> 2) | (data & 0x05);
                    self.prg[0] = val & 0x1F;
                }
                0x9 => match decoded_reg {
                    0 | 1 => {
                ...
                0xB..=0xE => {
                    let slot = (((bank - 0xB) << 1) | if bit1 { 1 } else { 0 }) as usize;
                    if bit0 {
                        let chr_val = ((data & 0x04) >> 1) | ((data & 0x02) << 1) | (data & 0x09);
                        self.chr[slot] =
                            (self.chr[slot] & 0x0F) | (((chr_val as u16) & 0x1F) << 4);
                    } else {
                        self.chr[slot] = (self.chr[slot] & 0x1F0) | ((data as u16) & 0x0F);
                    }
                }
                0xF => match decoded_reg {
                    0 => {
