...
impl Mapper128 {
    pub fn new() -> Self {
        Self { outer_bank: 0, latch_data: 0 }
    }
}
...
impl Mapper for Mapper128 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        ...
        let outer = (self.outer_bank >> 2) as usize;
        let num_16k = cart.prg_rom.len() / 0x4000;
        if num_16k == 0 {
            return FetchResult { data: 0, driven: false };
        }
        let bank = if address < 0xC000 {
            (self.latch_data as usize & 7) | outer
        } else {
            7 | outer
        } % num_16k;
    ...
    fn store_prg(&mut self, _cart: &mut Cartridge, address: u16, data: u8) {
        if address >= 0x8000 {
            if self.outer_bank < 0xF000 {
                self.outer_bank = address;
            }
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            ...
            let mirrored = if (self.outer_bank & 0x02) != 0 {
                (address & 0x33FF) | ((address & 0x0800) >> 1)
            } else {
                address & 0x37FF
            };
    ...
    fn save_mapper_registers(&self, _cart: &Cartridge) -> Vec<u8> {
        ...
        s.extend_from_slice(&self.outer_bank.to_le_bytes());
    ...
    fn load_mapper_registers(&mut self, _cart: &mut Cartridge, state: &[u8], start: usize) -> usize {
        let mut p = start;
        if p + 2 <= state.len() {
            self.outer_bank = u16::from_le_bytes([state[p], state[p + 1]]);
            p += 2;
        }
