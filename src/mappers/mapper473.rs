...
impl Mapper473 {
    ...
    fn prg_bank16(&self, cart: &Cartridge, cpu_bank: u8) -> u16 {
        let raw = self.prg_raw_bank(cart, cpu_bank) as u16;
        if (self.reg[0] & 0x20) != 0 {
            let shift = 7 - (self.reg[0] & 7);
            let prg_and = 0xFFu16 >> shift;
            let prg_or = (self.reg[1] as u16) | ((self.reg[2] as u16) << 8);
    ...
    fn chr_bank_1k(&self, address: u16) -> usize {
        if (self.reg[0] & 0x80) != 0 {
            self.mmc3.chr_bank(address) as usize
        } else {
            (self.reg[3] as usize) * 8 + (address as usize / 0x400)
        }
    }
}

impl Mapper for Mapper473 {
    fn reset(&mut self) {
        self.mmc3.reset();
        self.reg = [0; 4];
    }
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let bank = self.prg_bank16(cart, cpu_bank);
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if (0x4800..0x5000).contains(&address) {
            self.reg[(address & 3) as usize] = data;
        }
        if (0x8000..0xA000).contains(&address) && (self.reg[0] & 0x40) != 0 {
            return;
        }
    ...
    fn fetch_ppu(
        ...
        let bank1k = self.chr_bank_1k(address);
        let offset = bank1k * 0x400 + (address as usize & 0x3FF);
