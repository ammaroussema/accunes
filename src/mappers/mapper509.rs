...
impl Mapper for Mapper509 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            ...
            let offset = if (self.reg & 0x20) != 0 {
                ...
                let prg_and = if (self.reg & 0x10) != 0 { 0x1F } else { 0x0F };
                let prg_or = ((self.reg as usize) << 1) & !prg_and;
                let bank = (self.mmc3_raw_bank(cart, page) & prg_and) | prg_or;
                bank * 0x2000 + (address as usize & 0x1FFF)
            } else if (self.reg & 0x06) != 0 {
                let bank32 = (self.reg >> 1) as usize;
    ...
    fn store_prg(&mut self, cart: &mut Cartridge, address: u16, data: u8) {
        if (0x6000..0x8000).contains(&address) {
            if (self.reg & 0x20) == 0 && (self.mmc3.prg_ram_protect & 0x40) == 0 {
    ...
    fn fetch_ppu(
        ...
        let chr_and = if (self.reg & 0x10) != 0 { 0xFF } else { 0x7F };
        let chr_or = ((self.reg as usize) << 4) & !chr_and;
