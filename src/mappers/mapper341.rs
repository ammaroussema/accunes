...
impl Mapper341 {
    pub fn new(_header: &[u8], rom: &[u8], _rom_name: &str) -> Self {
        Self { latch_addr: 0, prg_rom_size: rom.len() }
    }
}
...
impl Mapper for Mapper341 {
    ...
    fn fetch_prg(&mut self, cart: &Cartridge, address: u16) -> FetchResult {
        if address >= 0x8000 {
            let bank = (self.latch_addr >> 8) as usize;
    ...
    fn mirror_nametable(&self, _cart: &Cartridge, address: u16) -> u16 {
        let bit = if self.prg_rom_size >= 0x40000 { 0x800 } else { 0x200 };
        mirror_h_or_v((self.latch_addr & bit) != 0, address)
    }
    ...
    fn fetch_ppu(
        ...
        if address < 0x2000 {
            ...
            let bit = if self.prg_rom_size >= 0x40000 { 0x800 } else { 0x200 };
            let mir = mirror_h_or_v((self.latch_addr & bit) != 0, address);
