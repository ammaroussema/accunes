...
#[allow(dead_code)]
pub const RC_CLIENT_LOG_LEVEL_INFO: i32 = 3;
pub const RC_CLIENT_LOG_LEVEL_VERBOSE: i32 = 4;
...
fn rc_client_get_hardcore_enabled(client: *const rc_client_opaque) -> i32;
    ...
    fn rc_client_enable_logging(
        client: *mut rc_client_opaque,
        level: i32,
        callback: RcMessageCallback,
    );
    ...
    fn rc_client_reset(client: *mut rc_client_opaque);
...
type RcMessageCallback = unsafe extern "C" fn(*const c_char, *const rc_client_opaque);

unsafe extern "C" fn log_callback(message: *const c_char, _client: *const rc_client_opaque) {
    if !message.is_null() {
        ...
        for b in 0..l.num_buckets as usize {
            ...
            for a in 0..bucket.num_achievements as usize {
                ...
                out.push(RaAchievement {
                    ...
                    badge_url: cstr_lossy(a.badge_url),
                    badge_locked_url: cstr_lossy(a.badge_locked_url),
                });
            }
        }
...
pub fn init_client() {
    ...
    unsafe {
        ...
        rc_client_enable_logging(client, RC_CLIENT_LOG_LEVEL_VERBOSE, log_callback);
...
pub fn set_hardcore(enabled: bool) {
    ...
    crate::config::save_ra_hardcore(enabled);
}
...
static ICON_CACHE: LazyLock<Mutex<HashMap<String, Arc<RaIcon>>>> = LazyLock::new(|| Mutex::new(HashMap::new()));
static ICON_FETCHING: LazyLock<Mutex<Vec<String>>> = LazyLock::new(|| Mutex::new(Vec::new()));

pub fn get_icon(url: &str) -> Option<Arc<RaIcon>> {
    if url.is_empty() {
        return None;
    }
    ICON_CACHE.lock().unwrap().get(url).cloned()
}

pub fn request_icon(url: &str) {
    if url.is_empty() {
        return;
    }
    {
    ...
    {
        let mut fetching = ICON_FETCHING.lock().unwrap();
        if fetching.iter().any(|u| u == url) {
            return;
        }
        fetching.push(url.to_string());
    }
    let url = url.to_string();
    std::thread::spawn(move || {
        let result = perform_http(&url, None);
        match result {
            Ok(bytes) => {
                let decoded = image::load_from_memory(&bytes).ok().map(|img| img.to_rgba8());
                if let Some(img) = decoded {
                    let (w, h) = img.dimensions();
                    let icon = Arc::new(RaIcon { data: img.to_vec(), w, h });
                    ICON_CACHE.lock().unwrap().insert(url.clone(), icon);
                }
            }
            Err(e) => {
                eprintln!("[RA] icon fetch failed for {}: {}", url, e);
            }
        }
        ICON_FETCHING.lock().unwrap().retain(|u| u != &url);
    });
}
